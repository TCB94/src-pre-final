...
$result = $db->query('SELECT u.email, u.title, u.url, u.location, u.use_avatar, u.signature, u.email_setting, u.num_posts, u.registered, u.admin_note, p.id, p.poster AS username, p.poster_id, p.poster_ip, p.poster_email, p.message, p.hide_smilies, p.posted, p.edited, p.edited_by, p.score, g.g_id, g.g_user_title, o.user_id AS is_online FROM '.$db->prefix.'posts AS p INNER JOIN '.$db->prefix.'users AS u ON u.id=p.poster_id INNER JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id LEFT JOIN '.$db->prefix.'online AS o ON (o.user_id=u.id AND o.user_id!=1 AND o.idle=0) WHERE p.topic_id='.$id.' ORDER BY p.id LIMIT '.$start_from.','.$pun_user['disp_posts'], true) or error('Unable to fetch post info', __FILE__, __LINE__, $db->error());
while ($cur_post = $db->fetch_assoc($result))
{
	$blocked = False;
	 if($cur_post['score'] >= TOO_MANY_POST_REPORTS){
		 $blocked = True;
		 $cur_post['message'] = "This post is currently under review because it was reported a number of times. Mods will check it out soon, and if it is okay it will be restored.";
	 }
	...
	if ($cur_post['poster_id'] > 1)
	{
		...
		$user_title = get_title($cur_post, false);
		...
		if ($pun_config['o_show_user_info'] == '1')
		{
			...
			{
				// If post > 100, show post count 'tiers' (i.e.'> 500 posts') instead of actual post count
				if ($cur_post['num_posts'] < 101)
				...
				else
				{
					if ($cur_post['num_posts'] > 1000)
						$post_count_shown = 1000;
					else if ($cur_post['num_posts'] > 500)
						$post_count_shown = 500;
					else if ($cur_post['num_posts'] > 100)
						$post_count_shown = 100;

					$user_info[] = '<dd><a href="search.php?action=show_user&user_id='.$cur_post['poster_id'].'">'.$lang_common['Posts'].'</a>: '.$post_count_shown.'+';
				}
	...
	else if (!($pun_user['g_id'] == PUN_ADMIN))  // If not an admin, must be a mod,  give them controls without delete
	{
		$post_actions[] = '<li class="postreport"><a href="misc.php?report='.$cur_post['id'].'">'.$lang_topic['Report'].'</a>'.$lang_topic['Link separator'].'</li><li class="postquote"><a href="movepost.php?id='.$cur_post['id'].'">'.$lang_topic['Move'].'</a>'.$lang_topic['Link separator'].'</li><li class="postedit"><a href="edit.php?id='.$cur_post['id'].'">'.$lang_topic['Edit'].'</a>'.$lang_topic['Link separator'].'</li><li class="postquote"><a href="post.php?tid='.$id.'&amp;qid='.$cur_post['id'].'">'.$lang_topic['Quote'].'</a>'; //Move Post 1.3 Mod row
	}
	else if($pun_user['g_id'] == PUN_ADMIN) // if admin, can delete
	{
...
<?php if (count($user_contacts)) echo "\t\t\t\t\t".'<dd class="usercontacts">'.implode('&nbsp;&nbsp;', $user_contacts).'</dd>'."\n"; ?>
			...
			<div class="postright <?php echo ($blocked) ? ' blocked' : ''; ?>">
...
<?php
$timestamp = $cur_post['registered'];
$created = date($pun_config['o_date_format'], $timestamp);
$current_date = date($pun_config['o_date_format']);

$date_diff = (strtotime($current_date) - strtotime($created))/(60*60*24);

?>				 
<?php if ($signature != '')
// Amos: Can check for group_id of poster here. If matches $pun_config['o_default_user_group'], don't show sig.
		...
		<form name="post" method="post" action="post.php?tid=<?php echo $id ?>" onsubmit="this.submit.disabled=true;if(process_form(this)){return true;}else{this.submit.disabled=false;return false;}">
			<div class="inform">
				<fieldset>
					...
					To add Scratch Blocks to your post, write out your scripts in between [scratchblocks] and [/scratchblocks], like in the following example. More information is available on <a href="http://wiki.scratch.mit.edu/wiki/Block_Plugin/Syntax" target="_BLANK">this page</a>.<br><br>
					<div id="blockexamplecode">
					[scratchblocks]<br>
					when gf clicked<br>
					repeat until &lt;(timer) &gt; [10]&gt;<br>
					&nbsp;&nbsp;go to [mouse-pointer v]<br>
					end<br>
					think [Scripts in your posts!] for (3) secs<br>
					[/scratchblocks]<br>
					</div>
					<pre id="blockexample" class="scratchblocks">
