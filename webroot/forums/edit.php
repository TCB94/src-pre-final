...
if (isset($_POST['form_sent']))
{
	...
	if ($pun_config['p_message_bbcode'] == '1' && strpos($message, '[') !== false && strpos($message, ']') !== false)
	{
        ...
        // Check if the post contains img tags, and throw an error if the host is not whitelisted.
	if (preg_match( '/\[\/img\]/i', $message)){

        	// Ensure pattern for each image matches whitelisted image hosting sites
		$imageHosts = IMAGE_HOST_WHITELIST_REGEX;

                preg_match_all('/\[img\](.*?)\[\/img\]/i', $message, $imgCheckMatches, PREG_SET_ORDER);
                foreach ($imgCheckMatches as $val) {
                	if (!preg_match( $imageHosts, $val[1])){
                        	$errors[] = IMAGE_HOST_NOT_WHITELISTED_ERROR.$val[1];
                                break;
                        }
                        else if (preg_match('/.*redirect\/url.*http.*/i', $val[1])){
                                $errors[] = "Sorry, you can't use redirects for image hosting. Please update your image links or remove all BB code [img] tags. Bad image url: ".$val[1];
                                break;
                        }
