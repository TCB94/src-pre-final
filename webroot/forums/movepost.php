...
/***********************************************************************
  ...
  code originally by Frédéric Pouget
  altered by eric235u
	...
	if ($form_sent > 1)
	{
		...
		else
		{
			if (isset($post_ids))
			{
				...
				{
					$db->query('UPDATE '.$db->prefix.'posts SET topic_id='.$new_topic_id.' WHERE id='.$post_id_this) or error('Unable to move the post', __FILE__, __LINE__, $db->error());
				}
			}
			else
			{
