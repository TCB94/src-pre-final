...
session_start();
$logged_into_scratchr=isset($_SESSION['User']);

if (isset($_GET['action'])||$logged_into_scratchr)
...
// Don't allow logins from banned Scratchers
if($_SESSION['User']['status'] == 'locked')
{
        redirect('/');
}
...
if ((isset($_REQUEST['form_sent']) && $action == 'in')||($logged_into_scratchr&&!isset($_GET['action'])))
{
	$form_username = trim($_REQUEST['req_username']);
	$form_password = trim($_REQUEST['req_password']);
	
    if($logged_into_scratchr){
        $form_username = trim($_SESSION['User']['username']);
        $form_password = trim("wRewat6usutu57ared79".$_SESSION['User']['username']); //random password which will be used for all scratchr users+the user's username
    }
	...
	$authorized = false;
    if(empty($db_password_hash)&&$logged_into_scratchr){ //the scratchr user doesn't have a forums account yet

	// Set the default user group to the value set in forum admin interface
        ...
        $db->query('INSERT INTO '.$db->prefix.'users (username, group_id, password, email, email_setting, save_pass, timezone, language, style, registered, registration_ip, last_visit) VALUES(\''.$db->escape($form_username).'\', '.$initial_group_id.', \''.$password_hash.'\', \''.$_SESSION['User']['email'].'\', 1, 1, '.$timezone.', \'English\', \''.$pun_config['o_default_style'].'\', '.$now.', \''.get_remote_address().'\', '.$now.')') or error('DB INSERT mess-up. Let nbushak@media.mit.edu know.',__FILE__,__LINE__,$db->error());
        $new_uid = $db->insert_id();

        //Copy and paste from above -- we re-run the DB request again, after adding the user
        $result = $db->query('SELECT id, group_id, password, save_pass FROM '.$db->prefix.'users WHERE '.$username_sql) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
        list($user_id, $group_id, $db_password_hash, $save_pass) = $db->fetch_row($result);
}
...
//We'll update forum user data from Scratch user data. The most important piece of data is whether or not the user is an admin.
if((!($group_id==1)&&($_SESSION['User']['role']=='admin'))||(($group_id==1)&&($_SESSION['User']['role']=='user'))){
if($_SESSION['User']['role']=='admin'){
$should_be_group_id=1;
}else{
    $should_be_group_id=5;
}

$db->query('UPDATE '.$db->prefix.'users SET group_id='.$should_be_group_id.' WHERE id='.$user_id) or error('Unable to update user admin group', __FILE__, __LINE__, $db->error());
$result = $db->query('SELECT id, group_id, password, save_pass FROM '.$db->prefix.'users WHERE '.$username_sql) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
list($user_id, $group_id, $db_password_hash, $save_pass) = $db->fetch_row($result);
}
}
	...
	redirect(htmlspecialchars($_REQUEST['redirect_url']), $lang_login['Login redirect']);
}


else if ($action == 'out')
{
	if ($pun_user['is_guest']) //removed: || !isset($_GET['id']) || $_GET['id'] != $pun_user['id']
	{
	...
	redirect(htmlspecialchars($_REQUEST['redirect_url']), $lang_login['Logout redirect']);}
	else{
