...
if (isset($_POST['zap_id']))
{
	...
	$result = $db->query('SELECT zapped, post_id FROM '.$db->prefix.'reports WHERE id='.$zap_id) or error('Unable to fetch report info', __FILE__, __LINE__, $db->error());
	...
	if ($zapped['zapped'] == ''){
		...
		$db->query('UPDATE '.$db->prefix.'posts SET score=0 WHERE id='.$zapped['post_id']) or error('Unable to clear post score', __FILE__, __LINE__, $db->error());
	}
...
// Unzap a report
// Code here is by Jacob G. (jvvg)
if (isset($_POST['unzap_id']))
{
	confirm_referrer('admin_reports.php');
	
	$zap_id = intval(key($_POST['unzap_id']));
	
	$result = $db->query('SELECT zapped, post_id FROM '.$db->prefix.'reports WHERE id='.$zap_id) or error('Unable to fetch report info', __FILE__, __LINE__, $db->error());
	$zapped = $db->fetch_assoc($result);

	if ($zapped['zapped'] != ''){
		$db->query('UPDATE '.$db->prefix.'reports SET zapped=NULL, zapped_by=NULL WHERE id='.$zap_id) or error('Unable to unzap report', __FILE__, __LINE__, $db->error());
	}
	
	redirect('admin_reports.php', 'Report unzapped. Redirecting &hellip;');
}
					...
					<fieldset>
						...
						<div class="infldset">
							<table cellspacing="0">
								...
								<tr>
									<th scope="row">Reported by <?php echo $reporter ?><div class="topspace">Zapped by <?php echo $zapped_by ?><div><input type="submit" name="unzap_id[<?php echo $cur_report['id'] ?>]" value=" Unzap " /></div></div></th>
									<td><?php echo $post ?></td>
								</tr>
							</table>
						</div>
